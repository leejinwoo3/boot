...
$('.register').click(function(){
    ...
    //라이도 버튼의 value를 가져오기 ->여러개를 한꺼번에 가져오기 때문에 each를 사용
    ...
    if(!email) { //이메일에 입력된게 없을때
        ...
        if(!emailCheck(email)){//이메일 형식이 맞지 않을때
    ...
    if(!password) { //패스워드에 입력된게 없을때
        ...
        if(!pwdCheck(password)) { //비밀 번호 형식에 맞지 않을때
            alert('특수문자 / 문자 / 숫자 포함 형태의 8~15자리 이내입니다.')
            return;
        }
    }
    
    if(!gender) { //성별 선택을 안했을 때 
        alert('성별을 입력해주세요!')
        return;
    }
